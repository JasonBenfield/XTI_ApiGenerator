...
export class AddEmployeeForm extends Form {
	constructor(private readonly vm: AddEmployeeFormViewModel) {
		...
		this.HireDate.setItems(
			new DropDownFieldItem(new Date(Date.UTC(2021, 0, 18, 5, 0, 0, 0)), 'Yesterday'),
			new DropDownFieldItem(new Date(Date.UTC(2021, 0, 19, 5, 0, 0, 0)), 'Today'),
			new DropDownFieldItem(new Date(Date.UTC(2021, 0, 20, 5, 0, 0, 0)), 'Tomorrow')
		);
